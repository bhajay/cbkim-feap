...
natoms=15 # excluding [0,0,0] atom
...
for tcheck in np.arange(trange):
  ...
  x_14 = np.dot(np.linalg.inv(A),r_14)
  lattice = [[a_x, a_y, a_z],
            [b_x, b_y, b_z],
            [c_x, c_y, c_z]]
  positions = [[float(x_0[0]), float(x_0[1]), float(x_0[2])],
              ...
              [float(x_14[0]), float(x_14[1]), float(x_14[2])]]

  if tcheck == tc1:
    r_7s = R[tcheck][7][:]*2
    r_8s = R[tcheck][8][:]*2
    x_7s = np.dot(np.linalg.inv(As),r_7s)
    x_8s = np.dot(np.linalg.inv(As),r_8s)
    x_7p1 = x_7s
    x_8p1 = x_8s
    #positions1 = [[float(x_7p1[0]-x_7p1[0]), float(x_7p1[1]-x_7p1[1]), float(x_7p1[2]-x_7p1[2])],
    #               [float(x_8p1[0]-x_7p1[0]), float(x_8p1[1]-x_7p1[1]), float(x_8p1[2]-x_7p1[1])]]
    positions1 = [[float(x_7p1[0]), float(x_7p1[1]), float(x_7p1[2])],
                   [float(x_8p1[0]), float(x_8p1[1]), float(x_8p1[2])]]
  if tcheck == tc2:
    r_7s = R[tcheck][7][:]*2
    r_8s = R[tcheck][8][:]*2
    x_7s = np.dot(np.linalg.inv(As),r_7s)
    ...
    positions2 = [[float(x_7p2[0]), float(x_7p2[1]), float(x_7p2[2])],
                   [float(x_8p2[0]), float(x_8p2[1]), float(x_8p2[2])]]
  numbers = [14,]*natoms
...
print('Scaled Positions of Atoms 9 and 10 at t(n=' + str(tc1) +') ...')
print(positions1)
print('Scaled Positions of Atoms 9 and 10 at t(n=' + str(tc2) +') ...')
print(positions2)
