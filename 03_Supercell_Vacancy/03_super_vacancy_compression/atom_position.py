...
nsteps=414
natoms=14
removedatom=13 # second to last atom in basis (15th atom removed in a set of 16)
atomposlinear1 = []
atomposlinear2 = []
atomposlinear3 = []
time = []
...
for i in np.arange(nrows):
  for j in np.arange(ncols):
    ...
    if cnt != removedatom:
      ...
      ax[i][j].set_title('Atom '+str(p+1), loc='left')
      ax[i][j].grid(True)
      ax[i][j].legend()
      cnt+=1
      p += 1
      ...
      ax[i][j].set_xlabel('Time')
      ax[i][j].set_ylabel('Position Component')
      ax[i][j].set_title('Atom '+str(removedatom+1) +'(removed)', loc='left')
      ax[i][j].grid(True)
      ax[i][j].legend()
      cnt += 1
