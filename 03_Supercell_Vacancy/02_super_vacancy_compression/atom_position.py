...
outfile = 'Osi_p3'
hisfile1 = "Psi_p3a.his"
hisfile2 = "Psi_p3b.his"
hisfile3 = "Psi_p3c.his"
nsteps=414
natoms=14
removedatom=13 # second to last atom in basis (15th atom removed in a set of 16 diamond cubic lattice)
atomposlinear1 = []
atomposlinear2 = []
atomposlinear3 = []
time = []
...
for i in np.arange(nrows):
  for j in np.arange(ncols):
    ...
    if cnt != removedatom:
      # don't plot in the respective location and mark it as removed. Also do the proper indexing to the apostimei (i=1,2,3) vecs
      ...
      ax[i][j].set_ylabel('Position Vector Component Value')
      ax[i][j].set_title('Atom '+str(cnt+1), loc='left') #indexing is due to Python count from zero
      ax[i][j].grid(True)
      ax[i][j].legend()
      cnt+=1
      p += 1
      ...
      ax[i][j].set_xlabel('Time')
      ax[i][j].set_ylabel('Position Vector Component Value')
      ax[i][j].set_title('Atom '+str(removedatom+1) + '(removed)', loc='left')  # indexing due to Python count from zero
      cnt+=1

# global plot specifications    
fig.subplots_adjust(hspace=.5)
fig.suptitle('Position Vector Components of Atoms in Supercell (Atom 14 Vacancy)')
fig.legend(['1 Component of Position', '2 Component of Position','3 Component of Position'], loc='upper right')
fig.set_size_inches(20,10)
fig.savefig('figures/Atom_Positions_Supercell_Vacancy_Atom15', dpi=100)
fig.show()
